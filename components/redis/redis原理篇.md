...
HyperLogLog 是一种概率数据结构，用于在恒定的内存大小下估计集合的基数（Cardinality，即不重复元素的数量）。

**原理核心：伯努利试验**

1.  **伯努利过程**：假设抛硬币，正面为1，反面为0。如果出现序列 `00...01`，其中有 $k$ 个0。根据概率统计，我们大概需要抛 $2^{k+1}$ 次硬币才能出现一次这样的序列。
2.  **基数估计**：如果我们在输入流中观察到的最大前导零个数为 $k_{max}$，那么可以估算输入流中不同元素的数量约为 $2^{k_{max}+1}$。

**Redis 中的实现优化**：

单纯使用一个最大前导零个数估算误差较大，HyperLogLog 采用了**分桶平均**的方法来减小误差：

1.  **分桶 (Bucketing)**：将哈希值分为 $m = 2^b$ 个桶。Redis 使用 16384 ($2^{14}$) 个桶。
2.  **哈希计算**：对输入元素进行 64 位哈希计算。
    *   低 14 位：用于定位桶的索引（Register Index）。
    *   高 50 位：用于计算前导零的个数（Run Length）。
3.  **调和平均 (Harmonic Mean)**：每个桶维护该桶内观察到的最大前导零个数。最终基数通过所有桶的调和平均数计算得出，并乘以一个修正系数。

**优势**：
*   **内存占用极小且固定**：无论基数多大，在 Redis 中每个 HyperLogLog 键只需占用 12KB 内存。
*   **误差可控**：标准误差约为 $0.81\%$。
